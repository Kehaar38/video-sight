...
```text
初期フレームサイズ: CIF 400x296
実出力: OV3660 set_res_raw() による 320x320
LCD表示: 320x320の中央240x284を等倍表示
SD保存: 処理を簡単にするため、フレーム全体の320x320 BMP
fb_count: 1
```
...
`240x240` は動作し、`240x284` と `320x320` は `cam_task` で落ちたため、次は定型サイズである `CIF 400x296` をセンサー側ROIとして出力した。
...
fb_count: 1 -> 2へ変更して再試行
```

実機確認結果:

- LCD全体に中央 `240x284` が表示された。
- 見かけの大きさは `240x240` ROIと大きく変わらない。
- 撮影保存も元サイズ `400x296` で成功した。
- ただし、`240x240` ROIよりフレームレートが低く、もっさり感が戻った。

シリアル上の保存時間:

```text
Saved /fov/fov_0000_400x296.bmp in 885 ms
Saved /fov/fov_0001_400x296.bmp in 884 ms
```

このもっさり感の主因は、LCD中央クロップ処理そのものより、センサー出力が `240x240` から `400x296` へ増え、DVP転送とフレーム配送量が約2倍になったことだと考える。LCD表示の中央クロップは等倍切り出しのため、汎用スケーラではなく直接行コピーに近い処理へ最適化する。
