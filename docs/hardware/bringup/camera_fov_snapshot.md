...
```text
camera frame: XGA 1024x768 / RGB565
SD保存: /fov/fov_0000_1024x768.bmp など
LCD表示: 1024x768中央から150mm視距離の1倍相当101x118をクロップし、240x284へ拡大表示
```
...
```text
source: 1024x768
crop:  center 101 x 118 相当
crop origin: x=(1024-101)/2, y=(768-118)/2
result: 240 x 284へ約2.4倍表示
```
...
QXGAおよびUXGA RGB565は `capture failed` となり映像が出なかったため、まずはXGAへ下げる。XGAでは150mm視距離の1倍に近い中央 `101 x 118px` をクロップする。VGA時の約3.8倍拡大より小さい、約2.4倍拡大で表示する初期値である。これにより、1m地点での見え幅はおおよそ次の値になる。

```text
クロップ: 101 x 118px
1m地点での見え幅: 約197 x 233mm
表示FOV: 水平 約11.24° / 垂直 約13.28°
LCDへの拡大率: 横 約2.38倍 / 縦 約2.41倍
```
...
保存BMPからカメラ生FOVを測定し、その後に1倍相当になる中央クロップ量を決める。2026-06-14時点ではXGA `101 x 118px` の中央クロップを初期値とする。
