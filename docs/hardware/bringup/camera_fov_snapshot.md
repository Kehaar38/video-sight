...
```text
camera frame: UXGA 1600x1200 / RGB565
SD保存: /fov/fov_0000_1600x1200.bmp など
LCD表示: 1600x1200中央から150mm視距離の1倍相当158x185をクロップし、240x284へ拡大表示
```
...
```text
source: 1600x1200
crop:  center 158 x 185 相当
crop origin: x=(1600-158)/2, y=(1200-185)/2
result: 240 x 284へ約1.5倍表示
```
...
QXGA RGB565の等倍クロップは `capture failed` となり映像が出なかったため、まずはUXGAへ下げる。UXGAでは150mm視距離の1倍に近い中央 `158 x 185px` をクロップする。VGA時の約3.8倍拡大より小さい、約1.5倍拡大で表示する初期値である。これにより、1m地点での見え幅はおおよそ次の値になる。

```text
クロップ: 158 x 185px
1m地点での見え幅: 約198 x 234mm
表示FOV: 水平 約11.28° / 垂直 約13.33°
LCDへの拡大率: 横 約1.52倍 / 縦 約1.54倍
```
...
保存BMPからカメラ生FOVを測定し、その後に1倍相当になる中央クロップ量を決める。2026-06-14時点ではUXGA `158 x 185px` の中央クロップを初期値とする。
