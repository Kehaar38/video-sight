...
LCD表示: 640x480中央から1倍相当の63x74をクロップし、240x284へ拡大表示
```
...
```text
source: 640x480
crop:  center 63 x 74 相当
crop origin: x=(640-63)/2, y=(480-74)/2
result: 240 x 284へ拡大表示
```
...
この結果から、1倍表示用のLCDライブビューでは中央 `63 x 74px` をクロップする。これは、目の距離を `150mm` としてピントが合う距離を優先した初期値である。これにより、1m地点での見え幅はおおよそ次の値になる。

```text
クロップ: 63 x 74px
1m地点での見え幅: 約197 x 234mm
表示FOV: 水平 約11.24° / 垂直 約13.33°
```
...
保存BMPからカメラ生FOVを測定し、その後に1倍相当になる中央クロップ量を決める。2026-06-14時点では `63 x 74px` の中央クロップを初期値とする。
