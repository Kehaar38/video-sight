...
LCD表示: 640x480中央から1倍相当の157x185をクロップし、240x284へ拡大表示
```
...
```text
source: 640x480
crop:  center 157 x 185 相当
crop origin: x=(640-157)/2, y=(480-185)/2
...
この結果から、1倍表示用のLCDライブビューでは中央 `157 x 185px` をクロップする。これにより、1m地点での見え幅はおおよそ次の値になる。

```text
クロップ: 157 x 185px
1m地点での見え幅: 約491 x 584mm
表示FOV: 水平 約27.57° / 垂直 約32.57°
```
...
保存BMPからカメラ生FOVを測定し、その後に1倍相当になる中央クロップ量を決める。2026-06-14時点では `157 x 185px` の中央クロップを初期値とする。
