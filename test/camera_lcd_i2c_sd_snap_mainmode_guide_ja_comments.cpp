...
// ===== LCDピン割り当て（docs/videosight_hardware_config_for_xiao_coding.md準拠） =====
...
static constexpr int LCD_ROTATION = 2;                  // 0..3（2=180度）

SPIClass lcdSpi(FSPI);
Adafruit_ST7789 tft(&lcdSpi, PIN_LCD_CS, PIN_LCD_DC, PIN_LCD_RST);

// ===== XIAO ESP32S3 Sense カメラピン定義（OV2640） =====
// カメラモジュールの版違いで初期化失敗する場合は、このピン定義を実機に合わせて調整する。
...
static void initLcd() {
  ...
  tft.cp437(true);  // 三角記号コードを使うためCP437を有効化
...
static void drawMainModeGuide() {
  ...
  // 背景透過にするため、塗りつぶしは行わず文字のみを重ね描きする。
  tft.setTextColor(c);
  tft.setTextSize(1);
